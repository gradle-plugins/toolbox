...
import static dev.gradleplugins.integtests.fixtures.nativeplatform.msvcpp.VisualStudioVersion.*;
...
public class AvailableToolChains {
    ...
    public abstract static class InstalledToolChain extends ToolChainCandidate {
        ...
        public void initialiseEnvironment() {
            String compilerPath = pathEntries.stream().map(File::getAbsolutePath).collect(joining(File.pathSeparator));
        ...
        protected List<String> toRuntimeEnv() {
            ...
            String path = pathEntries.stream().map(File::getAbsolutePath).collect(joining(File.pathSeparator)) + File.pathSeparator + System.getenv(pathVarName);
    ...
    static List<InstalledXcode> findXcodes() {
        ...
        List<File> xcodeCandidates = new ArrayList<>(Arrays.asList(GUtil.elvis(rootXcodeInstall.listFiles(xcodeInstall -> xcodeInstall.isDirectory()), new File[0])));
    ...
    public static class InstalledXcode {
        ...
        private List<String> getRuntimeEnv() {
            return Collections.singletonList("DEVELOPER_DIR=" + xcodeDir.getAbsolutePath());
        }
        ...
        private void configureExecuter(GradleExecuter executer) {
            executer.withEnvironmentVars(Collections.singletonMap("DEVELOPER_DIR", xcodeDir.getAbsolutePath()));
        }
