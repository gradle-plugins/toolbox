...
import dev.gradleplugins.fixtures.gradle.runner.parameters.BeforeExecute;

import java.util.ArrayList;
import java.util.List;

import static dev.gradleplugins.fixtures.gradle.runner.GradleRunnerParameters.allExecutionParameters;
...
//        GradleProvider effectiveDistribution = gradleProvider == null ? findGradleInstallFromGradleRunner() : gradleProvider;
    ...
    private GradleExecutionContext fireBeforeExecute(GradleExecutionContext parameters) {
        allExecutionParameters(parameters).stream().filter(it -> it instanceof BeforeExecute).forEach(action -> ((BeforeExecute) action).accept(parameters));
        return parameters;
    }
